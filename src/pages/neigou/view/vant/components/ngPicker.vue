 ...
 * @Last Modified time: 2018-06-01 11:13:09 
 */
<template>
    <div id="ng-picker">
        <div v-if="show">
            <select v-model="selectDay" name="" id="" >
                <option :selected="!value && !item.value" v-for="item in canUseDay" :value="item.value">{{item.label}}</option>
            </select>
        </div>
    </div>
</template>
...
export default {
    ...
    props:['value','date_ahead','invalid_dates'],
    mounted() {
        this.setUseDays()
    },
    methods:{
        setUseDays(){
            ...
            days.forEach(item => {
                ...
                if(this.date_ahead.indexOf(week)>=0||this.invalid_dates.indexOf(item)>=0){
                    
                }else{
                    this.canUseDay.push({"label":item,"value":item})
                }
            });
