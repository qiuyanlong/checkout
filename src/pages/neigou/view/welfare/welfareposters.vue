      ...
      <div class="month-count">
        <div class="all-c border-last" v-for="(list_voucher,index) in cont_list">
          <div class="month-list">
            ...
            <!--<h3>{{list_voucher.name}}：<span>￥</span>&nbsp;<em>{{list_voucher.money}}</em></h3>-->
...
<script>
  import './css/welfareposters.scss';
  ...
  export default {
    ...
    created() {
      ...
      httpPost(`${hdDomin}WelfareVoucherNotLogin/splashNext`,{encrypt_info:encrypt_info}).then(res =>{
//                console.log(res);
        if(res.data.errno == 0) {
